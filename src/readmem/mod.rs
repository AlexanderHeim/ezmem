...
pub fn read_bytes(handle: HANDLE, address: usize, amount: usize) -> Vec<u8> {
    ...
    unsafe {
        if ReadProcessMemory(handle, address as LPCVOID, bytes.as_ptr() as LPVOID, amount, std::ptr::null_mut()) == 0 {
...
fn read_primitive<T: Default>(handle: HANDLE, address: usize) -> T {
    ...
    unsafe {
        if ReadProcessMemory(handle, address as LPCVOID, &mut read_result as *mut _ as LPVOID, std::mem::size_of::<T>(), std::ptr::null_mut()) == 0 {
            panic!("ReadProcessMemory failed! Last OS Error: {}", GetLastError());
        }
    }
    read_result
...
pub fn resolve_multi_level_pointer(handle: HANDLE, base_ptr: usize, offsets: Vec<usize>) -> usize {
    ...
    unsafe {
        ...
        for i in 0..offsets.len() {
            buffer = 0;
            if ReadProcessMemory(handle, address as LPCVOID, &mut buffer as *mut _ as LPVOID, pointer_size, std::ptr::null_mut()) == 0 {
                panic!("ReadProcessMemory failed! Last OS Error: {}", GetLastError());
            }
