...
pub fn write_bytes(handle: HANDLE, address: usize, bytes: Vec<u8>) {
    unsafe {
        if WriteProcessMemory(handle, address as LPVOID, bytes.as_ptr() as LPVOID, bytes.len(), std::ptr::null_mut()) == 0 {
...
fn write_primitive<T>(handle: HANDLE, address: usize, to_write: T) {
    unsafe {
        if WriteProcessMemory(handle, address as LPVOID, &to_write as *const _ as LPCVOID, std::mem::size_of::<T>(), std::ptr::null_mut()) == 0 {
            panic!("Last OS Error: {}", GetLastError());
        }
    }
}
...
pub fn patch_ex(handle: HANDLE, address: usize, bytes: Vec<u8>) {
    ...
    unsafe {
        ...
        if VirtualProtectEx(handle, address as LPVOID, byte_amount, PAGE_EXECUTE_READWRITE, &mut last_protection as *mut _) == 0 {
            panic!("Last OS Error: {}", GetLastError());
        };
        write_bytes(handle, address, bytes);
        if VirtualProtectEx(handle, address as LPVOID, byte_amount, last_protection, &mut last_protection as *mut _) == 0 {
            panic!("Last OS Error: {}", GetLastError());
        };
    }
